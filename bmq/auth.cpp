...
bool BMQ::proxy_check_auth(int64_t conn_id, bool outgoing, const peer_info& peer,
        zmq::message_t& cmd, const cat_call_t& cat_call, std::vector<zmq::message_t>& data) {
    auto command = view(cmd);
    std::string reply;

    if (!cat_call.first) {
        BMQ_LOG(warn, "Invalid command '", command, "' sent by remote [", to_hex(peer.pubkey), "]/", peer_address(cmd));
        reply = "UNKNOWNCOMMAND";
    } else if (peer.auth_level < cat_call.first->access.auth) {
        BMQ_LOG(warn, "Access denied to ", command, " for peer [", to_hex(peer.pubkey), "]/", peer_address(cmd),
                ": peer auth level ", peer.auth_level, " < ", cat_call.first->access.auth);
        reply = "FORBIDDEN";
    } else if (cat_call.first->access.local_mn && !local_master_node) {
        BMQ_LOG(warn, "Access denied to ", command, " for peer [", to_hex(peer.pubkey), "]/", peer_address(cmd),
                ": that command is only available when this BMQ is running in master node mode");
        reply = "NOT_A_MASTER_NODE";
    } else if (cat_call.first->access.remote_mn && !peer.master_node) {
        BMQ_LOG(warn, "Access denied to ", command, " for peer [", to_hex(peer.pubkey), "]/", peer_address(cmd),
                ": remote is not recognized as a master node");
        reply = "FORBIDDEN_MN";
    } else if (cat_call.second->second /*is_request*/ && data.empty()) {
        BMQ_LOG(warn, "Received an invalid request for '", command, "' with no reply tag from remote [",
    ...
    try {
        ...
        BMQ_LOG(debug, "Couldn't send auth failure message ", reply, " to peer [", to_hex(peer.pubkey), "]/", peer_address(cmd), ": ", err.what());
    }

    return false;
}

void BMQ::set_active_mns(pubkey_set pubkeys) {
    if (proxy_thread.joinable()) {
        auto data = bt_serialize(detail::serialize_object(std::move(pubkeys)));
        detail::send_control(get_control_socket(), "SET_MNS", data);
    } else {
        proxy_set_active_mns(std::move(pubkeys));
    }
}
void BMQ::proxy_set_active_mns(std::string_view data) {
    proxy_set_active_mns(detail::deserialize_object<pubkey_set>(bt_deserialize<uintptr_t>(data)));
}
void BMQ::proxy_set_active_mns(pubkey_set pubkeys) {
    pubkey_set added, removed;
    for (auto it = pubkeys.begin(); it != pubkeys.end(); ) {
        auto& pk = *it;
        if (pk.size() != 32) {
            BMQ_LOG(warn, "Invalid private key of length ", pk.size(), " (", to_hex(pk), ") passed to set_active_mns");
            it = pubkeys.erase(it);
            continue;
        }
        if (!active_master_nodes.count(pk))
            added.insert(std::move(pk));
        ++it;
    }
    if (added.empty() && active_master_nodes.size() == pubkeys.size()) {
        BMQ_LOG(debug, "set_active_mns(): new set of MNs is unchanged, skipping update");
        return;
    }
    for (const auto& pk : active_master_nodes) {
        if (!pubkeys.count(pk))
            removed.insert(pk);
        if (active_master_nodes.size() + added.size() - removed.size() == pubkeys.size())
            break;
    }
    proxy_update_active_mns_clean(std::move(added), std::move(removed));
}

void BMQ::update_active_mns(pubkey_set added, pubkey_set removed) {
    if (proxy_thread.joinable()) {
        std::array<uintptr_t, 2> data;
        data[0] = detail::serialize_object(std::move(added));
        data[1] = detail::serialize_object(std::move(removed));
        detail::send_control(get_control_socket(), "UPDATE_MNS", bt_serialize(data));
    } else {
        proxy_update_active_mns(std::move(added), std::move(removed));
    }
}
void BMQ::proxy_update_active_mns(bt_list_consumer data) {
    ...
    proxy_update_active_mns(std::move(added), std::move(remed));
}
void BMQ::proxy_update_active_mns(pubkey_set added, pubkey_set removed) {
    ...
    for (auto it = removed.begin(); it != removed.end(); ) {
        const auto& pk = *it;
        if (pk.size() != 32) {
            BMQ_LOG(warn, "Invalid private key of length ", pk.size(), " (", to_hex(pk), ") passed to update_active_mns (removed)");
            it = removed.erase(it);
        } else if (!active_master_nodes.count(pk) || added.count(pk) /* added wins if in both */) {
            it = removed.erase(it);
        } else {
            ++it;
        }
    ...
    for (auto it = added.begin(); it != added.end(); ) {
        const auto& pk = *it;
        if (pk.size() != 32) {
            BMQ_LOG(warn, "Invalid private key of length ", pk.size(), " (", to_hex(pk), ") passed to update_active_mns (added)");
            it = added.erase(it);
        } else if (active_master_nodes.count(pk)) {
            it = added.erase(it);
        } else {
            ++it;
        }
...
void BMQ::proxy_update_active_mns_clean(pubkey_set added, pubkey_set removed) {
    BMQ_LOG(debug, "Updating MN auth status with +", added.size(), "/-", removed.size(), " pubkeys");

    // For anything we remove we want close the connection to the MN (if outgoing), and remove the
    // stored peer_info (incoming or outgoing).
    for (const auto& pk : removed) {
        ConnectionID c{pk};
        active_master_nodes.erase(pk);
        ...
        for (auto it = range.first; it != range.second; ) {
            ...
            it = peers.erase(it);
            if (outgoing) {
                BMQ_LOG(debug, "Closing outgoing connection to ", c);
    ...
    for (auto& pk : added)
        active_master_nodes.insert(std::move(pk));
}

void BMQ::process_zap_requests() {
    for (std::vector<zmq::message_t> frames; recv_message_parts(zap_auth, frames, zmq::recv_flags::dontwait); frames.clear()) {
...
#endif
            BMQ_LOG(debug, "Processing ZAP authentication request");
        ...
        if (frames.size() < 6 || view(frames[0]) != "1.0") {
            BMQ_LOG(error, "Bad ZAP authentication request: version != 1.0 or invalid ZAP message parts");
            status_code = "500";
            ...
            if (bind_id >= bind.size()) {
                BMQ_LOG(error, "Bad ZAP authentication request: invalid auth domain '", auth_domain, "'");
                status_code = "400";
                ...
                BMQ_LOG(error, "Bad ZAP authentication request: invalid ",
                ...
                BMQ_LOG(error, "Bad ZAP authentication request: invalid request pubkey");
                status_code = "500";
                ...
                if (bind[bind_id].curve) {
                    ...
                    mn = active_master_nodes.count(std::string{pubkey});
                }
                auto auth = bind[bind_id].allow(ip, pubkey, mn);
                ...
                if (auth <= AuthLevel::denied || auth > AuthLevel::admin) {
                    BMQ_LOG(info, "Access denied for incoming ", view(frames[5]), (mn ? " master node" : " client"),
                    ...
                    BMQ_LOG(debug, "Accepted incoming ", view(frames[5]), (mn ? " master node" : " client"),
        ...
        BMQ_TRACE("ZAP request result: ", status_code, " ", status_text);
