...
// Inside some method:
//     BMQ_LOG(warn, "bad ", 42, " stuff");
//
#define BMQ_LOG(level, ...) log(LogLevel::level, __FILE__, __LINE__, __VA_ARGS__)

#ifndef NDEBUG
// Same as BMQ_LOG(trace, ...) when not doing a release build; nothing under a release build.
#  define BMQ_TRACE(...) log(LogLevel::trace, __FILE__, __LINE__, __VA_ARGS__)
#else
#  define BMQ_TRACE(...)
#endif
...
constexpr char MN_ADDR_COMMAND[] = "inproc://mn-command";
constexpr char MN_ADDR_WORKERS[] = "inproc://mn-workers";
constexpr char MN_ADDR_SELF[] = "inproc://mn-self";
