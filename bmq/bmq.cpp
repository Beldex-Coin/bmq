...
void BMQ::set_zmq_context_option(zmq::ctxopt option, int value) {
    context.set(option, value);
}

void BMQ::log_level(LogLevel level) {
    log_lvl.store(level, std::memory_order_relaxed);
}

LogLevel BMQ::log_level() const {
    return log_lvl.load(std::memory_order_relaxed);
}


CatHelper BMQ::add_category(std::string name, Access access_level, unsigned int reserved_threads, int max_queue) {
...
void BMQ::add_command(const std::string& category, std::string name, CommandCallback callback) {
...
void BMQ::add_request_command(const std::string& category, std::string name, CommandCallback callback) {
    add_command(category, name, std::move(callback));
    categories.at(category).commands.at(name).second = true;
}

void BMQ::add_command_alias(std::string from, std::string to) {
...
zmq::socket_t& BMQ::get_control_socket() {
    assert(proxy_thread.joinable());

    // Optimize by caching the last value; BMQ is often a singleton and in that case we're
    ...
    if (proxy_shutting_down)
        throw std::runtime_error("Unable to obtain BMQ control socket: proxy thread is shutting down");
    ...
    if (!socket) {
        ...
        socket->connect(MN_ADDR_COMMAND);
    }
    last_id = object_id;
...
BMQ::BMQ(
        ...
        std::string privkey_,
        bool master_node,
        MNRemoteAddress lookup,
        Logger logger,
        LogLevel level)
    : object_id{next_id++}, pubkey{std::move(pubkey_)}, privkey{std::move(privkey_)}, local_master_node{master_node},
        mn_lookup{std::move(lookup)}, log_lvl{level}, logger{std::move(logger)}
{

    BMQ_TRACE("Constructing BMQ, id=", object_id, ", this=", this);

    if (sodium_init() == -1)
        throw std::runtime_error{"libsodium initialization failed"};

    if (pubkey.empty() != privkey.empty()) {
        throw std::invalid_argument("BMQ construction failed: one (and only one) of pubkey/privkey is empty. Both must be specified, or both empty to generate a key.");
    } else if (pubkey.empty()) {
        if (master_node)
            throw std::invalid_argument("Cannot construct a master node mode BMQ without a keypair");
        BMQ_LOG(debug, "generating x25519 keypair for remote-only BMQ instance");
        ...
        if (verify_pubkey != pubkey)
            throw std::invalid_argument("Invalid pubkey/privkey values given to BMQ construction: pubkey verification failed");
    }
}
...
void BMQ::start() {
    ...
    BMQ_LOG(info, "Initializing BMQ ", bind.empty() ? "remote-only" : "listener", " with pubkey ", to_hex(pubkey));
    ...
    if (MAX_SOCKETS > 1 && MAX_SOCKETS <= zmq_socket_limit)
        ...
        BMQ_LOG(error, "Not applying BMQ::MAX_SOCKETS setting: ", MAX_SOCKETS, " must be in [1, ", zmq_socket_limit, "]");
    ...
    // except binding it.
    command.bind(MN_ADDR_COMMAND);
    std::promise<void> startup_prom;
    auto proxy_startup = startup_prom.get_future();
    proxy_thread = std::thread{&BMQ::proxy_loop, this, std::move(startup_prom)};

    BMQ_LOG(debug, "Waiting for proxy thread to initialize...");
    proxy_startup.get(); // Rethrows exceptions from the proxy startup (e.g. failure to bind)

    BMQ_LOG(debug, "Waiting for proxy thread to get ready...");
    auto &control = get_control_socket();
    detail::send_control(control, "START");
    BMQ_TRACE("Sent START command");

    zmq::message_t ready_msg;
    std::vector<zmq::message_t> parts;
    try { recv_message_parts(control, parts); }
    catch (const zmq::error_t &e) { throw std::runtime_error("Failure reading from BMQ::Proxy thread: "s + e.what()); }
    ...
    BMQ_LOG(debug, "Proxy thread is ready");
}

void BMQ::listen_curve(std::string bind_addr, AllowFunc allow_connection, std::function<void(bool)> on_bind) {
...
void BMQ::listen_plain(std::string bind_addr, AllowFunc allow_connection, std::function<void(bool)> on_bind) {
    if (std::string_view{bind_addr}.substr(0, 9) == "inproc://")
...
std::pair<BMQ::category*, const std::pair<BMQ::CommandCallback, bool>*> BMQ::get_command(std::string& command) {
    if (command.size() > MAX_CATEGORY_LENGTH + 1 + MAX_COMMAND_LENGTH) {
        BMQ_LOG(warn, "Invalid command '", command, "': command too long");
        return {};
    }
    ...
    if (dot == 0 || dot == std::string::npos) {
        BMQ_LOG(warn, "Invalid command '", command, "': expected <category>.<command>");
        return {};
    }
    ...
    if (catit == categories.end()) {
        BMQ_LOG(warn, "Invalid command category '", catname, "'");
        return {};
    }
    ...
    if (callback_it == category.commands.end()) {
        BMQ_LOG(warn, "Invalid command '", command, "'");
        return {};
    }
...
void BMQ::set_batch_threads(int threads) {
    if (proxy_thread.joinable())
...
void BMQ::set_reply_threads(int threads) {
    if (proxy_thread.joinable())
...
void BMQ::set_general_threads(int threads) {
    if (proxy_thread.joinable())
...
BMQ::run_info& BMQ::run_info::load(category* cat_, std::string command_, ConnectionID conn_, Access access_, std::string remote_,
...
BMQ::run_info& BMQ::run_info::load(category* cat_, std::string command_, std::string remote_, std::function<void()> callback) {
    reset();
    is_injected = true;
    cat = cat_;
    command = std::move(command_);
...
BMQ::run_info& BMQ::run_info::load(pending_command&& pending) {
...
BMQ::run_info& BMQ::run_info::load(batch_job&& bj, bool reply_job, int tagged_thread) {
    reset();
    is_batch_job = true;
...
BMQ::~BMQ() {
    ...
    BMQ_LOG(info, "BMQ shutting down proxy thread");
    detail::send_control(get_control_socket(), "QUIT");
    proxy_thread.join();
    BMQ_LOG(info, "BMQ proxy thread has stopped");
}
