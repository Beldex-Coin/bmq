...
std::ostream& operator<<(std::ostream& o, const ConnectionID& conn) {
    if (!conn.pk.empty())
        return o << (conn.mn() ? "MN " : "non-MN authenticated remote ") << to_hex(conn.pk);
    else
...
void BMQ::setup_external_socket(zmq::socket_t& socket) {
...
void BMQ::setup_outgoing_socket(zmq::socket_t& socket, std::string_view remote_pubkey, bool use_ephemeral_routing_id) {
...
void BMQ::setup_incoming_socket(zmq::socket_t& listener, bool curve, std::string_view pubkey, std::string_view privkey, size_t bind_index) {
...
// Deprecated versions:
ConnectionID BMQ::connect_remote(std::string_view remote, ConnectSuccess on_connect,
...
void BMQ::disconnect(ConnectionID id, std::chrono::milliseconds linger) {
...
BMQ::proxy_connect_mn(std::string_view remote, std::string_view connect_hint, bool optional, bool incoming_only, bool outgoing_only, bool use_ephemeral_routing_id, std::chrono::milliseconds keep_alive) {
    ...
    if (peer) {
        BMQ_TRACE("proxy asked to connect to ", to_hex(remote), "; reusing existing connection");
        if (peer->route.empty() /* == outgoing*/) {
            if (peer->idle_expiry < keep_alive) {
                BMQ_LOG(debug, "updating existing outgoing peer connection idle expiry time from ",
        ...
        BMQ_LOG(debug, "proxy asked for optional or incoming connection, but no appropriate connection exists so aborting connection attempt");
        return {nullptr, ""s};
    }

    // No connection so establish a new one
    BMQ_LOG(debug, "proxy establishing new outbound connection to ", to_hex(remote));
    std::string addr;
    ...
    if (to_self) {
        ...
        if (addr.empty())
            addr = mn_lookup(remote);
        else
            BMQ_LOG(debug, "using connection hint ", connect_hint);

        if (addr.empty()) {
            BMQ_LOG(error, "peer lookup failed for ", to_hex(remote));
            return {nullptr, ""s};
        }
    }

    BMQ_LOG(debug, to_hex(pubkey), " (me) connecting to ", addr, " to reach ", to_hex(remote));
    ...
    try {
        ...
        BMQ_LOG(error, "Outgoing connection to ", addr, " failed: ", e.what());
        return {nullptr, ""s};
    }
    ...
    outgoing_mn_conns.emplace_hint(outgoing_mn_conns.end(), p.conn_id, ConnectionID{remote});
    auto it = connections.emplace_hint(connections.end(), p.conn_id, std::move(socket));

    return {&it->second, ""s};
}

std::pair<zmq::socket_t *, std::string> BMQ::proxy_connect_mn(bt_dict_consumer data) {
    ...
    if (!data.skip_until("pubkey"))
        throw std::runtime_error("Internal error: Invalid proxy_connect_mn command; pubkey missing");
    remote_pk = data.consume_string_view();

    return proxy_connect_mn(remote_pk, hint, optional, incoming_only, outgoing_only, ephemeral_rid, keep_alive);
}
...
void BMQ::proxy_close_connection(int64_t id, std::chrono::milliseconds linger) {
    auto it = connections.find(id);
    if (it == connections.end()) {
        BMQ_LOG(warn, "internal error: connection to close (", id, ") doesn't exist!");
        return;
    }
    BMQ_LOG(debug, "Closing conn ", id);
    it->second.set(zmq::sockopt::linger, linger > 0ms ? (int) linger.count() : 0);
    connections.erase(it);
    connections_updated = true;

    outgoing_mn_conns.erase(id);
}

void BMQ::proxy_expire_idle_peers() {
    for (auto it = peers.begin(); it != peers.end(); ) {
        auto &info = it->second;
        if (info.outgoing()) {
            auto idle = std::chrono::steady_clock::now() - info.last_activity;
            if (idle > info.idle_expiry) {
                BMQ_LOG(debug, "Closing outgoing connection to ", it->first, ": idle time (",
                ...
                BMQ_LOG(trace, "Not closing ", it->first, ": ", std::chrono::duration_cast<std::chrono::milliseconds>(idle).count(),
...
void BMQ::proxy_conn_cleanup() {
    BMQ_TRACE("starting proxy connections cleanup");

    // Drop idle connections (if we haven't done it in a while)
    BMQ_TRACE("closing idle connections");
    ...
    BMQ_TRACE("Timing out pending outgoing connections");
    ...
    BMQ_TRACE("Timing out pending requests");
    // Remove any expired pending requests and schedule their callback with a failure
    for (auto it = pending_requests.begin(); it != pending_requests.end(); ) {
        auto& callback = it->second;
        if (callback.first < now) {
            BMQ_LOG(debug, "pending request ", to_hex(it->first), " expired, invoking callback with failure status and removing");
    ...
    BMQ_TRACE("done proxy connections cleanup");
};

void BMQ::proxy_connect_remote(bt_dict_consumer data) {
    ...
    BMQ_LOG(debug, "Establishing remote connection to ", remote, remote_pubkey.empty() ? " (NULL auth)" : " via CURVE expecting pubkey " + to_hex(remote_pubkey));
    ...
    connections_updated = true;
    BMQ_LOG(debug, "Opened new zmq socket to ", remote, ", conn_id ", conn_id, "; sending HI");
...
void BMQ::proxy_disconnect(bt_dict_consumer data) {
    ...
    if (connid.mn() && connid.pk.size() != 32)
        throw std::runtime_error("Error: invalid disconnect of MN without a valid pubkey");

    proxy_disconnect(std::move(connid), linger);
}
void BMQ::proxy_disconnect(ConnectionID conn, std::chrono::milliseconds linger) {
    BMQ_TRACE("Disconnecting outgoing connection to ", conn);
    auto pr = peers.equal_range(conn);
    for (auto it = pr.first; it != pr.second; ++it) {
        auto& peer = it->second;
        if (peer.outgoing()) {
            BMQ_LOG(debug, "Closing outgoing connection to ", conn);
            proxy_close_connection(peer.conn_id, linger);
            peers.erase(it);
            return;
        }
    }
    BMQ_LOG(warn, "Failed to disconnect ", conn, ": no such outgoing connection");
}
