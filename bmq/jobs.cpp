...
void BMQ::proxy_batch(detail::Batch* batch) {
    batches.insert(batch);
    const auto [jobs, tagged_threads] = batch->size();
    BMQ_TRACE("proxy queuing batch job with ", jobs, " jobs", tagged_threads ? " (job uses tagged thread(s))" : "");
    if (!tagged_threads) {
...
void BMQ::job(std::function<void()> f, std::optional<TaggedThreadID> thread) {
...
void BMQ::proxy_schedule_reply_job(std::function<void()> f) {
    auto* b = new Batch<void>;
...
void BMQ::proxy_run_batch_jobs(std::queue<batch_job>& jobs, const int reserved, int& active, bool reply) {
...
void BMQ::proxy_timer(int id, std::function<void()> job, std::chrono::milliseconds interval, bool squelch, int thread) {
    if (!timers)
        timers.reset(zmq_timers_new());

    int zmq_timer_id = zmq_timers_add(timers.get(),
            interval.count(),
            [](int timer_id, void* self) { static_cast<BMQ*>(self)->_queue_timer_job(timer_id); },
            this);
    if (zmq_timer_id == -1)
        throw zmq::error_t{};
...
void BMQ::proxy_timer(bt_list_consumer timer_data) {
...
void BMQ::_queue_timer_job(int timer_id) {
    auto it = timer_jobs.find(timer_id);
    if (it == timer_jobs.end()) {
        BMQ_LOG(warn, "Could not find timer job ", timer_id);
        return;
    }
    auto& [func, squelch, running, thread] = it->second;
    if (squelch && running) {
        BMQ_LOG(debug, "Not running timer job ", timer_id, " because a job for that timer is still running");
        return;
    }

    if (thread == -1) { // Run directly in proxy thread
        try { func(); }
        catch (const std::exception &e) { BMQ_LOG(warn, "timer job ", timer_id, " raised an exception: ", e.what()); }
        catch (...) { BMQ_LOG(warn, "timer job ", timer_id, " raised a non-std exception"); }
        return;
    }
    ...
    batches.insert(b);
    BMQ_TRACE("b: ", b->size().first, ", ", b->size().second, "; thread = ", thread);
...
void BMQ::add_timer(TimerID& timer, std::function<void()> job, std::chrono::milliseconds interval, bool squelch, std::optional<TaggedThreadID> thread) {
...
TimerID BMQ::add_timer(std::function<void()> job, std::chrono::milliseconds interval, bool squelch, std::optional<TaggedThreadID> thread) {
    TimerID tid;
    add_timer(tid, std::move(job), interval, squelch, std::move(thread));
    return tid;
}

void BMQ::proxy_timer_del(int id) {
    if (!timers)
        return;
...
void BMQ::cancel_timer(TimerID timer_id) {
    if (proxy_thread.joinable()) {
...
void BMQ::TimersDeleter::operator()(void* timers) { zmq_timers_destroy(&timers); }

TaggedThreadID BMQ::add_tagged_thread(std::string name, std::function<void()> start) {
    if (proxy_thread.joinable())
    ...
    BMQ_TRACE("Created new tagged thread ", name, " with routing id ", run.worker_routing_id);

    run.worker_thread = std::thread{&BMQ::worker_thread, this, run.worker_id, name, std::move(start)};
