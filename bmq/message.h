...
class Message {
public:
    BMQ& bmq; ///< The owning BMQ object
    std::vector<std::string_view> data; ///< The provided command data parts, if any.
    ConnectionID conn; ///< The connection info for routing a reply; also contains the pubkey/mn status.
    ...
    /// Constructor
    Message(BMQ& bmq, ConnectionID cid, Access access, std::string remote)
        : bmq{bmq}, conn{std::move(cid)}, access{std::move(access)}, remote{std::move(remote)} {}
    ...
    /// with send_option::optional{} added if the originator is not a MN.  For MN messages (i.e.
    /// where `mn` is true) this is a "strong" reply by default in that the proxy will attempt to
    /// establish a new connection to the MN if no longer connected.  For non-MN messages the reply
    ...
    ///
    /// If you want to send a non-strong reply even when the remote is a master node then add
    ...
    /// bmq.request() that takes care of setting up the recipient arguments.
    ...
    public:
        ...
        explicit DeferredSend(Message& m) : bmq{m.bmq}, conn{m.conn}, reply_tag{m.reply_tag} {}
