...
void BMQ::proxy_quit() {
    BMQ_LOG(debug, "Received quit command, shutting down proxy thread");
    ...
    BMQ_LOG(debug, "Proxy thread teardown complete");
}

void BMQ::proxy_send(bt_dict_consumer data) {
    ...
    // multiple times, if we're sending to a MN, because it's possible that we have multiple
    // connections open to that MN (e.g. one out + one in) so if one fails we can clean up that
    ...
    while (retry) {
        ...
        if (conn_id.mn()) {
            auto sock_route = proxy_connect_mn(conn_id.pk, hint, optional, incoming, outgoing, EPHEMERAL_ROUTING_ID, keep_alive);
            if (!sock_route.first) {
                nowarn = true;
                if (optional)
                    BMQ_LOG(debug, "Not sending: send is optional and no connection to ",
                            to_hex(conn_id.pk), " is currently established");
                else
                    BMQ_LOG(error, "Unable to send to ", to_hex(conn_id.pk), ": no valid connection address found");
                break;
            }
            send_to = sock_route.first;
            conn_id.route = std::move(sock_route.second);
        } else if (!conn_id.route.empty()) { // incoming non-MN connection
            auto it = connections.find(conn_id.id);
            if (it == connections.end()) {
                BMQ_LOG(warn, "Unable to send to ", conn_id, ": incoming listening socket not found");
                break;
            }
            send_to = &it->second;
        } else {
            ...
            if (pr.first == peers.end()) {
                BMQ_LOG(warn, "Unable to send: connection id ", conn_id, " is not (or is no longer) a valid outgoing connection");
                break;
            }
            auto& peer = pr.first->second;
            auto it = connections.find(peer.conn_id);
            if (it == connections.end()) {
                BMQ_LOG(warn, "Unable to send: peer connection id ", conn_id, " is not (or is no longer) a valid outgoing connection");
                break;
            }
            send_to = &it->second;
        }
        ...
        try {
            ...
            if (e.num() == EHOSTUNREACH && !conn_id.route.empty() /*= incoming conn*/) {
                ...
                BMQ_LOG(debug, "Incoming connection is no longer valid; removing peer details");
                ...
                if (pr.first != peers.end()) {
                    if (!conn_id.mn()) {
                        ...
                        // The incoming connection to the MN is no longer good, but we can retry because
                        // we may have another active connection with the MN (or may want to open one).
                        if (removed) {
                            BMQ_LOG(debug, "Retrying sending to MN ", to_hex(conn_id.pk), " using other sockets");
                            retry = true;
                        }
                    }
                }
            ...
            if (!retry) {
                if (!conn_id.mn() && !conn_id.route.empty()) { // incoming non-MN connection
                    BMQ_LOG(debug, "Unable to send message to incoming connection ", conn_id, ": ", e.what(),
                            "; remote has probably disconnected");
                } else {
                    BMQ_LOG(warn, "Unable to send message to ", conn_id, ": ", e.what());
                }
                nowarn = true;
                if (callback_nosend) {
    ...
    if (request) {
        if (sent) {
            BMQ_LOG(debug, "Added new pending request ", to_hex(request_tag));
            pending_requests.insert({ request_tag, {
                std::chrono::steady_clock::now() + request_timeout, std::move(request_callback) }});
        } else {
            BMQ_LOG(debug, "Could not send request, scheduling request callback failure");
    ...
    if (!sent) {
        if (callback_nosend)
            ...
            BMQ_LOG(warn, "Unable to send message to ", conn_id, ": sending would block");
    }
}

void BMQ::proxy_reply(bt_dict_consumer data) {
    bool have_conn_id = false;
    ...
    if (pr.first == pr.second) {
        BMQ_LOG(warn, "Unable to send tagged reply: the connection is no longer valid");
        return;
    }

    // We try any connections until one works (for ordinary remotes there will be just one, but for
    // MNs there might be one incoming and one outgoing).
    for (auto it = pr.first; it != pr.second; ) {
        try {
            ...
            if (err.num() == EHOSTUNREACH) {
                if (it->second.outgoing()) {
                    BMQ_LOG(debug, "Unable to send reply to non-MN request on outgoing socket: "
                            "remote is no longer connected; closing connection");
                    proxy_close_connection(it->second.conn_id, CLOSE_LINGER);
                    it = peers.erase(it);
                    ++it;
                } else {
                    BMQ_LOG(debug, "Unable to send reply to non-MN request on incoming socket: "
                            "remote is no longer connected; removing peer details");
                    it = peers.erase(it);
                }
            } else {
                BMQ_LOG(warn, "Unable to send reply to incoming non-MN request: ", err.what());
                ++it;
            }
        }
    }
...
void BMQ::proxy_control_message(std::vector<zmq::message_t>& parts) {
    // We throw an uncaught exception here because we only generate control messages internally in
    // bmq code: if one of these condition fail it's a bmq bug.
    if (parts.size() < 2)
        throw std::logic_error("BMQ bug: Expected 2-3 message parts for a proxy control message");
    auto route = view(parts[0]), cmd = view(parts[1]);
    BMQ_TRACE("control message: ", cmd);
    if (parts.size() == 3) {
        BMQ_TRACE("...: ", parts[2]);
        auto data = view(parts[2]);
        if (cmd == "SEND") {
            BMQ_TRACE("proxying message");
            return proxy_send(data);
        } else if (cmd == "REPLY") {
            BMQ_TRACE("proxying reply to non-MN incoming message");
            return proxy_reply(data);
        } else if (cmd == "BATCH") {
            BMQ_TRACE("proxy batch jobs");
            auto ptrval = bt_deserialize<uintptr_t>(data);
            return proxy_batch(reinterpret_cast<detail::Batch*>(ptrval));
        } else if (cmd == "INJECT") {
            BMQ_TRACE("proxy inject");
            return proxy_inject_task(detail::deserialize_object<injected_task>(bt_deserialize<uintptr_t>(data)));
        } else if (cmd == "SET_MNS") {
            return proxy_set_active_mns(data);
        } else if (cmd == "UPDATE_MNS") {
            return proxy_update_active_mns(data);
        } else if (cmd == "CONNECT_MN") {
            proxy_connect_mn(data);
            return;
    ...
    throw std::runtime_error("BMQ bug: Proxy received invalid control command: " +
            std::string{cmd} + " (" + std::to_string(parts.size()) + ")");
}

bool BMQ::proxy_bind(bind_data& b, size_t bind_index) {
    ...
    if (!good) {
        BMQ_LOG(warn, "BMQ failed to listen on ", b.address);
        return false;
    }

    BMQ_LOG(info, "BMQ listening on ", b.address);
...
void BMQ::proxy_loop_init() {

#if defined(__linux__) || defined(__sun) || defined(__MINGW32__)
    pthread_setname_np(pthread_self(), "bmq-proxy");
#elif defined(__FreeBSD__) || defined(__NetBSD__) || defined(__OpenBSD__)
    pthread_set_name_np(pthread_self(), "bmq-proxy");
#elif defined(__MACH__)
    pthread_setname_np("bmq-proxy");
#endif
    ...
    workers_socket.bind(MN_ADDR_WORKERS);
    ...
    if (log_level() >= LogLevel::debug) {
        BMQ_LOG(debug, "Reserving space for ", max_workers, " max workers = ", general_workers, " general plus reservations for:");
        for (const auto& cat : categories)
            BMQ_LOG(debug, "    - ", cat.first, ": ", cat.second.reserved_threads);
        BMQ_LOG(debug, "    - (batch jobs): ", batch_jobs_reserved);
        BMQ_LOG(debug, "    - (reply jobs): ", reply_jobs_reserved);
        BMQ_LOG(debug, "Plus ", tagged_workers.size(), " tagged worker threads");
    }
...

    {
        ...
        inproc_listener.bind(MN_ADDR_SELF);
    ...
    for (size_t i = 0; i < bind.size(); i++) {
        if (!proxy_bind(bind[i], i)) {
            BMQ_LOG(fatal, "BMQ failed to listen on ", bind[i].address);
            throw zmq::error_t{};
        }
    }
    ...
    if (-1 == zmq_timers_add(timers.get(),
            ...
            [](int /*timer_id*/, void* self) { static_cast<BMQ*>(self)->proxy_conn_cleanup(); },
            this)) {
        throw zmq::error_t{};
    }
    ...
    if (!tagged_workers.empty()) {
        BMQ_LOG(debug, "Waiting for tagged workers");
        ...
        for (std::vector<zmq::message_t> parts; !waiting_on.empty(); parts.clear()) {
            ...
            if (parts.size() != 2 || view(parts[1]) != "STARTING"sv) {
                BMQ_LOG(error, "Received invalid message on worker socket while waiting for tagged thread startup");
                continue;
            }
            BMQ_LOG(debug, "Received STARTING message from ", view(parts[0]));
            if (auto it = waiting_on.find(view(parts[0])); it != waiting_on.end())
                waiting_on.erase(it);
            else
                BMQ_LOG(error, "Received STARTING message from unknown worker ", view(parts[0]));
        }

        for (auto&w : tagged_workers) {
            BMQ_LOG(debug, "Telling tagged thread worker ", std::get<run_info>(w).worker_routing_id, " to finish startup");
            route_control(workers_socket, std::get<run_info>(w).worker_routing_id, "START");
        }
    }
}

void BMQ::proxy_loop(std::promise<void> startup) {
    try {
        proxy_loop_init();
    } catch (...) {
    ...
    while (true) {
        ...
        if (proxy_skip_one_poll)
            ...
            BMQ_TRACE("polling for new messages");
        ...
        BMQ_TRACE("processing control messages");
        ...
        for (size_t pos = 0; pos != end; ++pos %= queue.size()) {
            ...
            if (parts.empty()) {
                BMQ_LOG(warn, "Ignoring empty (0-part) incoming message");
                continue;
            }
            ...
            if (connections_updated) {
                ...
                BMQ_TRACE("connections became stale; short-circuiting incoming message loop");
                break;
            }
        }

        BMQ_TRACE("done proxy loop");
    }
}

static bool is_error_response(std::string_view cmd) {
    return cmd == "FORBIDDEN" || cmd == "FORBIDDEN_MN" || cmd == "NOT_A_MASTER_NODE" || cmd == "UNKNOWNCOMMAND" || cmd == "NO_REPLY_TAG";
}

// Return true if we recognized/handled the builtin command (even if we reject it for whatever
// reason)
bool BMQ::proxy_handle_builtin(int64_t conn_id, zmq::socket_t& sock, std::vector<zmq::message_t>& parts) {
    ...
    if (parts.size() < 1 + incoming) {
        BMQ_LOG(warn, "Received empty message; ignoring");
        return true;
    }
    if (incoming) {
        route = view(parts[0]);
    ...
    BMQ_TRACE("Checking for builtins: '", cmd, "' from ", peer_address(parts.back()));

    if (cmd == "REPLY") {
        size_t tag_pos = 1 + incoming;
        if (parts.size() <= tag_pos) {
            BMQ_LOG(warn, "Received REPLY without a reply tag; ignoring");
            return true;
        }
        std::string reply_tag{view(parts[tag_pos])};
        auto it = pending_requests.find(reply_tag);
        if (it != pending_requests.end()) {
            BMQ_LOG(debug, "Received REPLY for pending command ", to_hex(reply_tag), "; scheduling callback");
            ...
            BMQ_LOG(warn, "Received REPLY with unknown or already handled reply tag (", to_hex(reply_tag), "); ignoring");
        }
        return true;
    } else if (cmd == "HI") {
        if (!incoming) {
            BMQ_LOG(warn, "Got invalid 'HI' message on an outgoing connection; ignoring");
            return true;
        }
        BMQ_LOG(debug, "Incoming client from ", peer_address(parts.back()), " sent HI, replying with HELLO");
        try {
            send_routed_message(sock, std::string{route}, "HELLO");
        } catch (const std::exception &e) { BMQ_LOG(warn, "Couldn't reply with HELLO: ", e.what()); }
        return true;
    } else if (cmd == "HELLO") {
        if (incoming) {
            BMQ_LOG(warn, "Got invalid 'HELLO' message on an incoming connection; ignoring");
            return true;
        }
        ...
        if (it == pending_connects.end()) {
            BMQ_LOG(warn, "Got invalid 'HELLO' message on an already handshaked incoming connection; ignoring");
            return true;
        }
        auto& pc = *it;
        auto pit = peers.find(std::get<int64_t>(pc));
        if (pit == peers.end()) {
            BMQ_LOG(warn, "Got invalid 'HELLO' message with invalid conn_id; ignoring");
            return true;
        }

        BMQ_LOG(debug, "Got initial HELLO server response from ", peer_address(parts.back()));
        ...
        if (!incoming) {
            BMQ_LOG(debug, "BYE command received; disconnecting from ", peer_address(parts.back()));
            proxy_close_connection(conn_id, 0s);
        } else {
            BMQ_LOG(warn, "Got invalid 'BYE' command on an incoming socket; ignoring");
        }
        ...
        if (parts.size() == (1 + incoming) && cmd == "UNKNOWNCOMMAND") {
            ...
            // FORBIDDEN_MN: so log it only at debug level and move on.
            BMQ_LOG(debug, "Received plain UNKNOWNCOMMAND; remote is probably an older bmq. Ignoring.");
            return true;
        }
        ...
        if (parts.size() == (3 + incoming) && view(parts[1 + incoming]) == "REPLY") {
            ...
            auto it = pending_requests.find(reply_tag);
            if (it != pending_requests.end()) {
                BMQ_LOG(debug, "Received ", cmd, " REPLY for pending command ", to_hex(reply_tag), "; scheduling failure callback");
            ...
            BMQ_LOG(warn, "Received ", cmd, ':', (parts.size() > 1 + incoming ? view(parts[1 + incoming]) : "(unknown command)"sv),
