...
bool worker_wait_for(BMQ& bmq, zmq::socket_t& sock, std::vector<zmq::message_t>& parts, const std::string_view worker_id, const std::string_view expect) {
    while (true) {
        bmq.log(LogLevel::trace, __FILE__, __LINE__, "worker ", worker_id, " waiting for ", expect);
        parts.clear();
        recv_message_parts(sock, parts);
        if (parts.size() != 1) {
            bmq.log(LogLevel::error, __FILE__, __LINE__, "Internal error: worker ", worker_id, " received invalid ", parts.size(), "-part control msg");
            continue;
        }
        auto command = view(parts[0]);
        if (command == expect) {
#ifndef NDEBUG
            bmq.log(LogLevel::trace, __FILE__, __LINE__, "Worker ", worker_id, " received waited-for ", expect, " command");
#endif
            return true;
        } else if (command == "QUIT"sv) {
            bmq.log(LogLevel::debug, __FILE__, __LINE__, "Worker ", worker_id, " received QUIT command, shutting down");
            detail::send_control(sock, "QUITTING");
            sock.set(zmq::sockopt::linger, 1000);
            sock.close();
            return false;
        } else {
            bmq.log(LogLevel::error, __FILE__, __LINE__, "Internal error: worker ", worker_id, " received invalid command: `", command, "'");
        }
    }
}
...
void BMQ::worker_thread(unsigned int index, std::optional<std::string> tagged, std::function<void()> start) {
    ...
    [[maybe_unused]] std::string thread_name = tagged.value_or("bmq-" + routing_id);
    ...
    BMQ_LOG(debug, "New worker thread ", worker_id, " (", routing_id, ") started");
    sock.connect(MN_ADDR_WORKERS);
    if (tagged)
    ...
    if (tagged) {
        // If we're a tagged worker then we got started up before BMQ started, so we need to wait
        // for an all-clear signal from BMQ first, then we fire our `start` callback, then we can
        ...
        try {
            if (run.is_batch_job) {
                ...
                if (run.batch_jobno >= 0) {
                    BMQ_TRACE("worker thread ", worker_id, " running batch ", batch, "#", run.batch_jobno);
                    batch->run_job(run.batch_jobno);
                } else if (run.batch_jobno == -1) {
                    BMQ_TRACE("worker thread ", worker_id, " running batch ", batch, " completion");
                    batch->job_completion();
                }
            } else if (run.is_injected) {
                auto& func = var::get<std::function<void()>>(run.to_run);
                BMQ_TRACE("worker thread ", worker_id, " invoking injected command ", run.command);
                func();
                func = nullptr;
            } else {
                ...
                BMQ_TRACE("Got incoming command from ", message.remote, "/", message.conn, message.conn.route.empty() ? " (outgoing)" : " (incoming)");
                ...
                BMQ_TRACE("worker thread ", worker_id, " invoking ", run.command, " callback with ", message.data.size(), " message parts");
                callback(message);
            }
        }
        catch (const bt_deserialize_invalid& e) {
            BMQ_LOG(warn, worker_id, " deserialization failed: ", e.what(), "; ignoring request");
        }
#ifndef BROKEN_APPLE_VARIANT
        catch (const std::bad_variant_access& e) {
            BMQ_LOG(warn, worker_id, " deserialization failed: found unexpected serialized type (", e.what(), "); ignoring request");
        }
#endif
        catch (const std::out_of_range& e) {
            BMQ_LOG(warn, worker_id, " deserialization failed: invalid data - required field missing (", e.what(), "); ignoring request");
        }
        catch (const std::exception& e) {
            BMQ_LOG(warn, worker_id, " caught exception when processing command: ", e.what());
        }
        catch (...) {
            BMQ_LOG(warn, worker_id, " caught non-standard exception when processing command");
        }
...
BMQ::run_info& BMQ::get_idle_worker() {
...
void BMQ::proxy_worker_message(std::vector<zmq::message_t>& parts) {
    // Process messages sent by workers
    if (parts.size() != 2) {
        BMQ_LOG(error, "Received send invalid ", parts.size(), "-part message");
        return;
    }
    auto route = view(parts[0]), cmd = view(parts[1]);
    BMQ_TRACE("worker message from ", route);
    ...
    if (!worker_id_str.empty() /* didn't consume everything */ ||
        ...
        BMQ_LOG(error, "Worker id '", route, "' is invalid, unable to process worker command");
        return;
    }

    auto& run = tagged_worker ? std::get<run_info>(tagged_workers[worker_id - 1]) : workers[worker_id];

    BMQ_TRACE("received ", cmd, " command from ", route);
    if (cmd == "RAN"sv) {
        BMQ_TRACE("Worker ", route, " finished ", run.is_batch_job ? "batch job" : run.command);
        if (run.is_batch_job) {
            if (tagged_worker) {
            ...
            if (run.batch_jobno == -1) {
                ...
                if (state == detail::BatchState::complete) {
                    if (thread == -1) { // run directly in proxy
                        BMQ_TRACE("Completion job running directly in proxy");
                        try {
                            ...
                            BMQ_LOG(error, "proxy thread caught exception when processing in-proxy completion command: ", e.what());
                        } catch (...) {
                            BMQ_LOG(error, "proxy thread caught non-standard exception when processing in-proxy completion command");
                        }
                        clear_job = true;
                    } else {
                        auto& jobs =
        ...
        if (max_workers == 0) { // Shutting down
            BMQ_TRACE("Telling worker ", route, " to quit");
        ...
        BMQ_LOG(debug, "Worker ", route, " exited normally");
    } else {
        BMQ_LOG(error, "Worker ", route, " sent unknown control message: `", cmd, "'");
    }
}

void BMQ::proxy_run_worker(run_info& run) {
...
void BMQ::proxy_to_worker(int64_t conn_id, zmq::socket_t& sock, std::vector<zmq::message_t>& parts) {
    ...
    if (outgoing) {
        auto snit = outgoing_mn_conns.find(conn_id);
        auto it = snit != outgoing_mn_conns.end()
            ? peers.find(snit->second)
            : peers.find(conn_id);

        if (it == peers.end()) {
            BMQ_LOG(warn, "Internal error: connection id ", conn_id, " not found");
            return;
        }
        peer = &it->second;
        ...
        tmp_peer.master_node = active_master_nodes.count(pubkey);
        peer = &tmp_peer;
    } else {
        std::tie(tmp_peer.pubkey, tmp_peer.auth_level) = detail::extract_metadata(parts.back());
        tmp_peer.master_node = tmp_peer.pubkey.size() == 32 && active_master_nodes.count(tmp_peer.pubkey);

        if (tmp_peer.master_node) {
            // It's a master node so we should have a peer_info entry; see if we can find one with
            ...
            if (!peer) {
                // We don't have a record: this is either a new MN connection or a new message on a
                // connection that recently gained MN status.
                peer = &peers.emplace(ConnectionID{tmp_peer.pubkey}, std::move(tmp_peer))->second;
            }
        } else {
            // Incoming, non-MN connection: we don't store a peer_info for this, so just use the
            // temporary one
            peer = &tmp_peer;
        }
    }
    ...
    Access access{peer->auth_level, peer->master_node, local_master_node};
    ...
    if (category.active_threads >= category.reserved_threads && active_workers() >= general_workers) {
        ...
        if (category.max_queue >= 0 && category.queued >= category.max_queue) {
            BMQ_LOG(warn, "No space to queue incoming command ", command, "; already have ", category.queued,
                    "commands queued in that category (max ", category.max_queue, "); dropping message");
            return;
        }

        BMQ_LOG(debug, "No available free workers, queuing ", command, " for later");
        ConnectionID conn{peer->master_node ? ConnectionID::MN_ID : conn_id, peer->pubkey, std::move(tmp_peer.route)};
    ...
    if (cat_call.second->second /*is_request*/ && data_parts.empty()) {
        BMQ_LOG(warn, "Received an invalid request command with no reply tag; dropping message");
        return;
    }

    auto& run = get_idle_worker();
    {
        ConnectionID c{peer->master_node ? ConnectionID::MN_ID : conn_id, peer->pubkey};
        c.route = std::move(tmp_peer.route);
        if (outgoing || peer->master_node)
    ...
    BMQ_TRACE("Forwarding incoming ", run.command, " from ", run.conn, " @ ", peer_address(parts[command_part_index]),
...
void BMQ::inject_task(const std::string& category, std::string command, std::string remote, std::function<void()> callback) {
    if (!callback) return;
...
void BMQ::proxy_inject_task(injected_task task) {
    auto& category = task.cat;
    if (category.active_threads >= category.reserved_threads && active_workers() >= general_workers) {
        // No free worker slot, queue for later
        if (category.max_queue >= 0 && category.queued >= category.max_queue) {
            BMQ_LOG(warn, "No space to queue injected task ", task.command, "; already have ", category.queued,
                    "commands queued in that category (max ", category.max_queue, "); dropping task");
            return;
        }
        BMQ_LOG(debug, "No available free workers for injected task ", task.command, "; queuing for later");
    ...
    auto& run = get_idle_worker();
    BMQ_TRACE("Forwarding incoming injected task ", task.command, " from ", task.remote, " to worker ", run.worker_routing_id);
