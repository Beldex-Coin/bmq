...
TEST_CASE("tcp QR-code friendly addresses", "[address][tcp][qr]") {
    address a{"tcp://explorer.beldex.io:12345"};
    address a_qr{"TCP://EXPLORER.BELDEX.IO:12345"};
    address b{"tcp://EXPLORER.BELDEX.IO:12345"};
    REQUIRE( a == a_qr );
    REQUIRE( a != b );
    REQUIRE( a.host == "explorer.beldex.io" );
    REQUIRE( a.qr_address() == "TCP://EXPLORER.BELDEX.IO:12345" );

    address c = address::tcp_curve("explorer.beldex.io", 12345, pk);
    REQUIRE( c.qr_address() == "CURVE://EXPLORER.BELDEX.IO:12345/" + pk_B32Z );
    REQUIRE( address{"CURVE://EXPLORER.BELDEX.IO:12345/" + pk_B32Z} == c );
    // We don't produce with upper-case hex, but we accept it:
    REQUIRE( address{"CURVE://EXPLORER.BELDEX.IO:12345/" + pk_HEX} == c );

    // lower case not permitted:                          ▾
    REQUIRE_THROWS_AS(address{"CURVE://EXPLORER.BELDEX.IO:12345/" + pk_B32Z}, std::invalid_argument);
    // also only accept upper-base base32z and hex:
    REQUIRE_THROWS_AS(address{"CURVE://EXPLORER.BELDEX.IO:12345/" + pk_b32z}, std::invalid_argument);
    REQUIRE_THROWS_AS(address{"CURVE://EXPLORER.BELDEX.IO:12345/" + pk_hex}, std::invalid_argument);
    // don't accept base64 even if it's upper-case (because case-converting it changes the value)
    REQUIRE_THROWS_AS(address{"CURVE://EXPLORER.BELDEX.IO:12345/AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"}, std::invalid_argument);
    REQUIRE_THROWS_AS(address{"CURVE://EXPLORER.BELDEX.IO:12345/AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="}, std::invalid_argument);
}

TEST_CASE("address hashing", "[address][hash]") {
    address a{"tcp://explorer.beldex.io:12345"};
    address b{"tcp+curve://explorer.beldex.io:12345/" + pk_hex};
    address c{"ipc:///tmp/some.sock"};
    address d{"ipc:///tmp/some.other.sock"};

    std::hash<bmq::address> hasher{};
    ...
    std::unordered_set<bmq::address> set;
    set.insert(a);
    set.insert(b);
    set.insert(c);
    set.insert(d);
    ...
    CHECK( set.size() == 4 );
    std::unordered_map<bmq::address, int> count;
