...
TEST_CASE("bt_value deserialization", "[bt][deserialization][bt_value]") {
    ...
    REQUIRE( bmq::get_int<int>(dna1) == 42 );
    REQUIRE( bmq::get_int<int>(dna2) == -42 );
    REQUIRE( bmq::get_int<unsigned>(dna1) == 42 );
    REQUIRE_THROWS( bmq::get_int<unsigned>(dna2) );
    ...
    for (auto& v : foo1a) foo1a_vals.push_back(bmq::get_int<int>(v));
    for (auto& v : foo1b) foo1b_vals.push_back(bmq::get_int<int>(v));
    for (auto& v : foo2c) foo2c_vals.push_back(bmq::get_int<int>(v));
...
#ifdef BMQ_APPLE_TO_CHARS_WORKAROUND
TEST_CASE("apple to_chars workaround test", "[bt][apple][sucks]") {
    ...
    CHECK( buf_view(bmq::apple_to_chars10(buf, 0)) == "0" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, 1)) == "1" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, 2)) == "2" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, 10)) == "10" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, 42)) == "42" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, 99)) == "99" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, 1234567890)) == "1234567890" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, -1)) == "-1" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, -2)) == "-2" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, -10)) == "-10" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, -99)) == "-99" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, -1234567890)) == "-1234567890" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, char{42})) == "42" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, (unsigned char){42})) == "42" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, short{42})) == "42" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, std::numeric_limits<char>::min())) == "-128" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, std::numeric_limits<char>::max())) == "127" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, (unsigned char){42})) == "42" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, std::numeric_limits<uint64_t>::max())) == "18446744073709551615" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, int64_t{-1})) == "-1" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, std::numeric_limits<int64_t>::min())) == "-9223372036854775808" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, int64_t{-9223372036854775807})) == "-9223372036854775807" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, int64_t{9223372036854775807})) == "9223372036854775807" );
    CHECK( buf_view(bmq::apple_to_chars10(buf, int64_t{9223372036854775806})) == "9223372036854775806" );
}
#endif
