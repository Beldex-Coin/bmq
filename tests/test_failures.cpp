...
TEST_CASE("failure responses - NOT_A_MASTER_NODE", "[failure][NOT_A_MASTER_NODE]") {
    std::string listen = random_localhost();
    BMQ server{
        "", "", // generate ephemeral keys
        false, // not a master node
    ...
    auto recvd = client.recv(resp);
    {
        auto lock = catch_lock();
        REQUIRE( recvd );
        REQUIRE( resp.to_string() == "NOT_A_MASTER_NODE" );
        REQUIRE( resp.more() );
        REQUIRE( client.recv(resp) );
        REQUIRE( resp.to_string() == "x.x" );
        REQUIRE_FALSE( resp.more() );
...
TEST_CASE("failure responses - FORBIDDEN_MN", "[failure][FORBIDDEN_MN]") {
    std::string listen = random_localhost();
    BMQ server{
        "", "", // generate ephemeral keys
        false, // not a master node
    ...
    auto recvd = client.recv(resp);
    {
        auto lock = catch_lock();
        REQUIRE( recvd );
        REQUIRE( resp.to_string() == "FORBIDDEN_MN" );
        REQUIRE( resp.more() );
        REQUIRE( client.recv(resp) );
        REQUIRE( resp.to_string() == "x.x" );
        REQUIRE_FALSE( resp.more() );
