...
TEST_CASE("tagged thread start functions", "[tagged][start]") {
    ...
    bmq.set_general_threads(2);
    bmq.set_batch_threads(2);
    auto t_abc = bmq.add_tagged_thread("abc");
    std::atomic<bool> start_called = false;
    auto t_def = bmq.add_tagged_thread("def", [&] { start_called = true; });
...
TEST_CASE("tagged threads quit-before-start", "[tagged][quit]") {
    auto bmq = std::make_unique<bmq::BMQ>(get_logger(""), LogLevel::trace);
    auto t_abc = bmq->add_tagged_thread("abc");
    REQUIRE_NOTHROW(bmq.reset());
}
...
TEST_CASE("batch jobs to tagged threads", "[tagged][batch]") {
    ...
    auto t_abc = bmq.add_tagged_thread("abc", [&] { id_abc = std::this_thread::get_id(); });
    auto t_def = bmq.add_tagged_thread("def", [&] { id_def = std::this_thread::get_id(); });
    bmq.start();

    std::atomic<bool> done = false;
    std::thread::id id;
    bmq.job([&] { id = std::this_thread::get_id(); done = true; });
    ...
    bmq.job([&] { id = std::this_thread::get_id(); done = true; }, t_abc);
    wait_for([&] { return done.load(); });
    {
        auto lock = catch_lock();
        REQUIRE( id == id_abc );
    ...
    bmq.job([&] { id = std::this_thread::get_id(); done = true; }, t_def);
    wait_for([&] { return done.load(); });
    {
        auto lock = catch_lock();
        REQUIRE( id == id_def );
    ...
    std::vector<int> v;
    for (int i = 0; i < 100; i++) {
        bmq.job([&] { if (std::this_thread::get_id() == id_abc) v.push_back(v.size()); }, t_abc);
    }
    bmq.job([&] { done = true; }, t_abc);
    wait_for([&] { return done.load(); });
    {
        auto lock = catch_lock();
        REQUIRE( done.load() );
...
TEST_CASE("batch job completion on tagged threads", "[tagged][batch-completion]") {
    ...
    bmq.set_general_threads(4);
    bmq.set_batch_threads(4);
...
TEST_CASE("timer job completion on tagged threads", "[tagged][timer]") {
    ...
    bmq.add_timer([&] { ticks++; }, 10ms);
    bmq.add_timer([&] { if (std::this_thread::get_id() == id_abc) abc_ticks++; }, 10ms, true, t_abc);
